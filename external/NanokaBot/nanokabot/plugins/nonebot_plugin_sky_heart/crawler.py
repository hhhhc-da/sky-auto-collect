...
class WebCrawler:
    ...
    def crawl_main(self, url="http://vip.gyzhax.cn/gy?key=SqIsr71MpxZL", headless=True):
        """单次请求爬虫主函数, 不要太频繁地请求网页就可以"""
        self.code = '暂未获取到好友码'  # 重置 code
        exp_flag = False 
        try:
            ...
            while True: # 如果繁忙那我们就一直扫就可以了，反正别的链接也会是繁忙的
                ...
                if not ok_button.exists:
                    print("没有找到确定按钮, 鉴定为出现了恶意取心")
                    raise Exception("没有找到确定按钮, 应该是被鉴定为恶意取心")
