...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default=os.path.join("dataset", "sky_human_v2", "data.yaml"), help='Path to the data configuration file.')
...
def train_yolov11n_model(options=None):
    ...
    path = options.data if options is not None else os.path.join("dataset", "sky_human_v2", "data.yaml")
    weights = options.weights if options is not None else os.path.join("weights", "yolo11s.pt")
